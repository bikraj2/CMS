...
from .formsets import ModuleFormSet
...
class ContentCreateUpdateView(TemplateResponseMixin,View):
    ...
    ## Simply just get the model assocaited with this content (text,video,file,image) 

    def get_model(self,model_name):
        if model_name in ['text','video','image','file']:
            return apps.get_model(app_label='courses',model_name=model_name)
        return None
    def get_from(self,model,*args,**kwargs):
        Form = modelform_factory(
            model, exclude=['owner','order','created','udpated']
        )
        return Form(*args,**kwargs)
    def dispatch(self, request, module_id,model_name,id = None,*args, **kwargs):
        self.module = get_object_or_404(Module,id=module_id,course__owner = request.user)
        self.model = apps.get_model(app_label='courses',model_name=model_name)
        if id :
            self.obj = get_object_or_404(self.model,id=id,owner=request.user)
        return super().dispatch(request, module_id,model_name)
    def get(self,request_module_id,model_name,id=None):
        form = self.get_from(self.model,instance = self.obj)
        return self.render_to_response(
...
class ModuleContentListView(TemplateResponseMixin,View):
    template_name = 'course/manage/module/content__list.html'
...
class CourseListView(TemplateResponseMixin,View):
    ...
    def get(self,request,subject=None):
        ...
        if subject:
            ...
            if not courses:
                courses = all_courses.filter(subject=subject) # filtering courses with given subject 
